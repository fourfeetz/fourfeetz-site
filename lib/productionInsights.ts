...
function makeGuide(seed: GuideSeed): ProductionInsight {
  ...
  return {
    ...
    sections: [
      ...
      {
        id: "production-notes",
        title: "FourFeetz Production Notes: Review Scope and Controls",
        paragraphs: [
          `The public review scope for this workflow is ${seed.reviewScope}. This describes the decisions visible in the guide and linked production material; it is not presented as a time sheet, generation ledger or universal benchmark.`,
          `Budgeting depends on the current service and the amount of material that passes review. ${seed.budgetNote} Live plan, credit, access and usage terms must be checked before production because private invoices and character-specific weighting remain confidential.`,
          `The public controls are ${seed.recommendedSettings.join("; ")}. They are review prompts for beginner-to-intermediate work, not a master preset. We change a control only after writing the visible failure it is meant to solve.`,
        ],
        table: {
          title: `${seed.shortTitle}: public production notes`,
          headers: ["Review layer", "Public note", "Why it matters"],
          rows: [
            ["Character focus", seed.characters.join(" + "), "Prevents every guide from becoming another generic HARU example"],
            ["Review scope", seed.reviewScope, "Makes the visible evidence and decision boundaries clear"],
            ["Budget consideration", seed.budgetNote, "Avoids presenting changing service terms as a fixed studio cost"],
      ...
      },
      {
        ...
        paragraphs: [
          `Our public workflow for ${seed.topic} moves through intention, evidence, a shot or asset card, controlled testing, continuity review, an edit check and finishing. A gate can send the work backward. That is healthy. Returning to an unstable reference is more efficient than repairing a batch of clips generated from it. Returning to scene planning avoids grading a shot that has no usable cut point.`,
      ...
      },
      {
        ...
        paragraphs: [
          ...
          `The best-looking single output is not automatically the best production choice. We compare continuity burden, how clearly a failure can be diagnosed, edit flexibility and delivery quality. A method with predictable revisions can be more useful than fast generation that produces unrelated successes.`,
      ...
      },
      {
        ...
        paragraphs: [
          `Our educational prompt structure moves from subject state and source evidence to one observable action, camera or composition behavior, and light or finish. We use concrete nouns and verbs. Words such as cinematic, premium and emotional are supporting direction, not substitutes for physical description.`,
      ...
      },
      {
        ...
        paragraphs: [
          ...
          `Name versions by question rather than mood. “Slow push — identity check” is more useful than “beautiful final new.” Save the source, tool or method, date, controlled change, result and rejection reason. A usable archive allows a future scene to benefit from today’s failure.`,
...
const seeds: GuideSeed[] = [
  {
    ...
    reviewScope: "beat-sheet intent, shot cards, motion candidates, rough-edit continuity and final approval were reviewed as separate stages",
    budgetNote: "Generation was gated by shot approval, with finishing reserved for material that already passed the rough edit.",
  ...
  },
  {
    ...
    reviewScope: "body motion, character identity, a camera alternate and the edit decision were assessed in separate passes",
    budgetNote: "Short candidates were reviewed before any hero treatment; current Runway credits and plan limits must be checked live.",
  ...
  },
  {
    ...
    reviewScope: "the handoff frame, continuation, planned end frame and edit-only alternative were reviewed against the same storyboard destination",
    budgetNote: "Only the failing layer was retested; model, duration and output choices can change live credit use.",
...
export function createProductionInsightMetadata(source: ProductionInsightSlug | ProductionInsight): Metadata {
  ...
  return {
    ...
    openGraph: {
      ...
      description: article.description,
      url: canonical,
      images: [{ url: article.hero, alt: article.heroAlt ?? `${article.shortTitle} by FourFeetz Studios` }],
