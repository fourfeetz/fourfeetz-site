...
export function KoreanHomePage() {
  ...
  return <main>
    ...
    <section className="px-6 py-20"><SectionHeading eyebrow="Character Universe" title="오리지널 캐릭터" description="서로 다른 개성과 이야기를 가진 FourFeetz의 오리지널 캐릭터들을 만나보세요." /><div className="mx-auto mt-10 grid max-w-7xl gap-5 sm:grid-cols-2 lg:grid-cols-3">{characterDetails.map((item) => <MediaCard key={item.slug} href={`/ko/characters/${item.slug}`} image={item.gallery[0]?.image} title={koreanCharacters[item.slug].displayName ?? item.name} category={koreanCharacters[item.slug].species} description={koreanCharacters[item.slug].tagline} portrait fullyClickable />)}</div></section>
...
export function KoreanAboutPage() {
  const cards = [
    ...
    { title: "경험에서 나온 제작 가이드", copy: "단순한 AI 뉴스 복제보다 직접 제작하고 테스트하며 확인한 문제, 선택과 결과를 크리에이터가 활용할 수 있는 인사이트로 정리합니다.", icon: BookOpen },
  ];
  return <main><PageHero eyebrow="FourFeetz Studios" title="따뜻한 캐릭터와 이야기를 만드는 AI 애니메이션 스튜디오" desc="FourFeetz는 캐릭터 중심의 감성 영상, 음악과 제작 인사이트를 함께 만드는 창작 브랜드입니다." illustration={{ src: "/images/about-hero-v2.png", alt: "AI 영상 작업 중인 FourFeetz 스튜디오와 HARU" }} />
    <section className="px-6 pb-20"><div className="mx-auto grid max-w-7xl gap-8 rounded-[32px] border border-[#eadfce] bg-[#fffdf8] p-8 md:grid-cols-[0.7fr_1.3fr] md:p-10"><div><p className="text-sm font-black uppercase tracking-[0.28em] text-[#a67c52]">About the Studio</p><h2 className="mt-3 text-3xl font-black text-[#2b2119] md:text-4xl">직접 만든 결과물로 이야기합니다.</h2></div><div className="space-y-4 text-lg leading-8 text-[#76685d]"><p>FourFeetz는 오리지널 동물 캐릭터를 중심으로 AI 애니메이션 필름, 세로형 쇼츠와 음악을 만드는 독립 창작 스튜디오입니다. 한 번 보고 지나가는 영상보다 캐릭터의 인상과 감정이 작품마다 자연스럽게 이어지는 세계를 지향합니다.</p><p>사이트에 공개하는 제작 가이드와 도구 분석은 실제 영상 제작과 테스트에서 얻은 경험을 바탕으로 합니다. 새로운 기능이나 모델 소식도 발표 내용을 반복하는 데 그치지 않고, 제작자에게 어떤 의미가 있는지 구분해 다룹니다.</p></div></div></section>
...
export function KoreanResourcesPage() {
  return <main><PageHero eyebrow="Creator Resources" title="리소스" desc="FourFeetz 실제 제작 경험에서 정리한 공개 가이드, 템플릿과 체크리스트로 AI 영상 제작을 계획하고 검토하세요." illustration={{ src: "/images/resources-hero-v2.png", alt: "노트북, 워크플로 카드, 템플릿과 체크리스트가 있는 FourFeetz AI 영상 제작 리소스" }} /><section className="px-6 pb-12"><div className="mx-auto max-w-7xl rounded-[32px] border border-[#eadfce] bg-white p-8 md:p-10"><h2 className="text-3xl font-black text-[#2b2119] md:text-4xl">재현 가능한 판단을 공개합니다.</h2><p className="mt-5 max-w-4xl text-lg leading-8 text-[#76685d]">장면 목적, 기준 이미지, 실패 원인과 검토 결과를 기록하는 데 사용할 수 있는 공개 자료입니다. FourFeetz 캐릭터를 그대로 재현하는 마스터 프롬프트, 내부 바이블이나 최신 핵심 설정은 포함하지 않습니다.</p></div></section><section className="border-y border-[#eadfce] bg-white px-6 py-20"><div className="mx-auto grid max-w-7xl gap-6 md:grid-cols-2 lg:grid-cols-3">{resourceDetails.map((item) => <MediaCard key={item.slug} href={`/ko/resources/${item.slug}`} image={item.image} title={item.title} category={item.category} description={koreanResourceDescriptions[item.slug]} fullyClickable />)}</div></section><PracticalResourceCards language="ko" /></main>;
}

export function KoreanToolsPage() {
  const tools = [
    { name: "Google Flow / Google AI Pro", role: "승인된 이미지를 바탕으로 한 영상 생성, 장면 연장과 절제된 움직임 테스트에 사용합니다.", limit: "First Shot 검토가 필요하며, 행동을 많이 넣으면 얼굴·발·방 구조가 달라질 수 있습니다." },
    { name: "Runway", role: "초기 HARU 제작에서 이미지 투 비디오, Multi-Shot과 장면 연결 테스트에 사용했습니다.", limit: "좋은 단일 클립도 승인된 기준 이미지와 편집 연속성 비교를 대신하지 않습니다." },
    { name: "Kling AI", role: "End Frame, Continue, 클립 연장과 특정 장면 수정 범위에서 사용했습니다.", limit: "시작과 끝 상태가 분명할 때 유용하며 불안정한 원본 프레임 자체를 고치지는 못합니다." },
    { name: "ChatGPT", role: "이야기 구조, 샷 카드, 제작 노트 정리, 공개 글과 웹사이트 작업에 사용합니다.", limit: "작성 결과는 실제 이미지, 영상과 프로젝트 기록에 맞춰 확인한 뒤 공개합니다." },
    { name: "Suno", role: "FourFeetz 영상의 오리지널 배경 음악과 감정 큐를 탐색하는 데 사용합니다.", limit: "생성 후에도 장면에 맞는 선별, 타이밍과 앰비언스를 위한 믹스가 필요합니다." },
    { name: "CapCut", role: "장면 배열, 자르기, 짧은 디졸브, 사운드 균형, 화면 비율과 최종 출력에 사용합니다.", limit: "달라진 얼굴, 사라진 발이나 깨진 배경처럼 구조적인 생성 실패는 고칠 수 없습니다." },
  ];
  return <main>
    ...
    <section className="border-y border-[#eadfce] bg-white px-6 py-20"><div className="mx-auto grid max-w-7xl gap-6 md:grid-cols-2 lg:grid-cols-3">{tools.map((tool) => <article key={tool.name} className={`${cardClass} bg-[#fffdf8] p-7`}><p className="text-sm font-black uppercase tracking-[0.18em] text-[#a67c52]">Production Tool</p><h2 className="mt-3 text-2xl font-black text-[#2b2119]">{tool.name}</h2><p className="mt-5 leading-8 text-[#76685d]">{tool.role}</p><div className="mt-5 rounded-2xl border border-[#eadfce] bg-white p-5"><p className="text-sm font-black text-[#6f4e37]">확인한 한계</p><p className="mt-2 leading-7 text-[#76685d]">{tool.limit}</p></div></article>)}</div></section>
