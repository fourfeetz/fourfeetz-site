...
export function KoreanAboutPage() {
  ...
  return <main><PageHero eyebrow="FourFeetz Studios" title="따뜻한 캐릭터와 이야기를 만드는 AI 애니메이션 스튜디오" desc="FourFeetz는 캐릭터 중심의 감성 영상, 음악과 제작 인사이트를 함께 만드는 창작 브랜드입니다." illustration={{ src: "/images/about-hero-v2.png", alt: "AI 영상 작업 중인 FourFeetz 스튜디오와 HARU" }} />
    ...
    <section className="border-y border-[#eadfce] bg-white px-6 py-20"><SectionHeading eyebrow="Explore FourFeetz" title="작품과 제작 경험을 둘러보세요." description="캐릭터의 설정부터 완성된 영상, 실제 제작 과정에서 얻은 인사이트까지 서로 연결해 확인할 수 있습니다." /><div className="mx-auto mt-10 grid max-w-7xl gap-5 md:grid-cols-3">{[
      { title: "캐릭터", copy: "HARU, PORI, LUNA, HUGO, RURU와 MILO의 특징과 관련 콘텐츠를 소개합니다.", href: "/ko/characters" },
...
export function KoreanCharactersPage() {
  return <main><PageHero eyebrow="Original Character Universe" title="캐릭터" desc="FourFeetz의 필름, 쇼츠와 음악을 이어가는 여섯 오리지널 동물 캐릭터를 소개합니다." illustration={{ src: "/images/characters-hero-v2.png", alt: "FourFeetz 오리지널 동물 캐릭터들이 모인 스튜디오 장면" }} />
    <section className="px-6 pb-12"><div className="mx-auto max-w-7xl rounded-[32px] border border-[#eadfce] bg-white p-8 md:p-10"><p className="max-w-4xl text-lg leading-8 text-[#76685d]">FourFeetz의 캐릭터 세계는 첫 주인공 HARU에서 시작해 서로 다른 속도와 시선을 가진 친구들로 확장됩니다. PORI는 밝은 움직임을, LUNA는 조용한 관찰을, HUGO는 초원의 여유를 더합니다. RURU는 숲속의 호기심을 보여주며 MILO는 낙천적인 에너지로 세계의 폭을 넓힙니다. 각 카드를 선택하면 외형과 역할, 실제로 공개된 관련 작품을 확인할 수 있습니다.</p><div className="mt-7 flex flex-wrap gap-3"><Link href="/ko/videos?type=long-form" className={primaryButton}>롱폼에서 만나기</Link><Link href="/ko/videos?type=shorts" className={secondaryButton}>쇼츠 둘러보기</Link></div></div></section>
    <section className="px-6 pb-20"><div className="mx-auto grid max-w-7xl gap-6 sm:grid-cols-2 lg:grid-cols-3">{characterDetails.map((item) => { const ko = koreanCharacters[item.slug]; return <MediaCard key={item.slug} href={`/ko/characters/${item.slug}`} image={`/images/characters/${item.slug}/portrait.png`} imageAlt={ko.imageAlt} title={item.name} category={ko.species} description={ko.listing} portrait />; })}</div></section>
  </main>;
}
...
export function KoreanCharacterDetail({ slug }: { slug: string }) {
  ...
  return <main><section className="mx-auto grid max-w-7xl gap-10 px-6 py-16 md:grid-cols-2 md:items-center"><div><p className="text-sm font-black uppercase tracking-[0.28em] text-[#a67c52]">Original Character</p><h1 className="mt-4 text-6xl font-black text-[#2b2119] md:text-7xl">{item.name}</h1><p className="mt-4 text-2xl font-black text-[#6f4e37]">{ko.species}</p><p className="mt-6 text-lg leading-8 text-[#76685d]">{ko.tagline}</p></div><div className="relative aspect-square overflow-hidden rounded-[32px] border border-[#eadfce] bg-white"><Image src={`/images/characters/${slug}/portrait.png`} alt={ko.imageAlt} fill priority sizes="(min-width: 768px) 50vw, 100vw" className="object-contain p-5" /></div></section>
    ...
    <section className="px-6 py-20"><SectionHeading eyebrow="Related Stories" title={slug === "milo" ? "FourFeetz 세계 둘러보기" : `${item.name} 관련 콘텐츠`} description={slug === "milo" ? "MILO가 등장하는 개별 영상과 음악은 아직 공개되지 않았습니다. 현재 공개된 캐릭터와 필름을 먼저 만나보세요." : "실제로 공개된 영상과 음악에서 캐릭터의 표정, 움직임과 분위기를 확인해 보세요."} /><div className="mx-auto mt-10 grid max-w-7xl gap-5 md:grid-cols-2 lg:grid-cols-3">{ko.relatedContent.map((content) => <Link key={content.href} href={content.href} className={`${cardClass} p-7`}><p className="text-sm font-black text-[#a67c52]">{content.label}</p><h3 className="mt-3 text-2xl font-black text-[#2b2119]">{content.title}</h3><p className="mt-4 leading-7 text-[#76685d]">{content.description}</p><span className="mt-6 inline-flex font-black text-[#6f4e37]">관련 페이지 보기 →</span></Link>)}</div></section>
