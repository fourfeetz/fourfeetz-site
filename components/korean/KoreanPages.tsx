...
export function KoreanCharactersPage() {
  return <main><PageHero eyebrow="Original Character Universe" title="캐릭터" desc="FourFeetz의 필름, 쇼츠와 음악을 이어가는 일곱 오리지널 동물 캐릭터를 소개합니다." illustration={{ src: "/images/characters-hero-v2.png", alt: "FourFeetz 오리지널 동물 캐릭터들이 모인 스튜디오 장면" }} />
    <section className="px-6 pb-12"><div className="mx-auto max-w-7xl rounded-[32px] border border-[#eadfce] bg-white p-8 md:p-10"><p className="max-w-4xl text-lg leading-8 text-[#76685d]">FourFeetz의 캐릭터 세계는 첫 주인공 HARU에서 시작해 서로 다른 속도와 시선을 가진 친구들로 확장됩니다. PORI는 밝은 움직임을, LUNA는 조용한 관찰을, HUGO는 초원의 여유를 더합니다. RURU는 숲속의 호기심을, OLI는 물길을 따라가는 영리한 모험심을 보여주며 MILO는 낙천적인 에너지로 세계의 폭을 넓힙니다. 각 카드를 선택하면 외형과 역할, 공개된 경우 관련 작품을 확인할 수 있습니다.</p><div className="mt-7 flex flex-wrap gap-3"><Link href="/ko/videos?type=long-form" className={primaryButton}>롱폼에서 만나기</Link><Link href="/ko/videos?type=shorts" className={secondaryButton}>쇼츠 둘러보기</Link></div></div></section>
...
export function KoreanCharacterDetail({ slug }: { slug: string }) {
  ...
  const hasPublishedMedia = slug !== "milo" && slug !== "oli";
