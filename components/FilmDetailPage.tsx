...
export default function FilmDetailPage({ film }: { film: Film }) {
  ...
  return (
    <main>
      <section className="mx-auto grid max-w-7xl gap-10 px-6 py-16 md:grid-cols-[0.92fr_0.88fr] md:items-center md:py-20">
        <div>
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            ...
            {film.characterSlugs?.map((slug) => <Link key={slug} href={`/characters/${slug}`} className="rounded-full border border-[#6f4e37]/40 bg-white px-6 py-3 font-black text-[#6f4e37] transition hover:border-[#6f4e37]">Meet {film.character}</Link>)}
          </div>
        </div>
      ...
      {film.featuredRelease ? <section className="px-6 pb-20"><SectionHeader eyebrow="Published Release" title="Related Character Short" /><Link href={film.featuredRelease.href} className="mx-auto mt-8 block max-w-7xl rounded-[40px] border border-[#eadfce] bg-white p-8 shadow-xl shadow-[#6f4e37]/10 transition hover:-translate-y-1 hover:shadow-2xl md:p-12"><p className="text-sm font-black uppercase tracking-[0.35em] text-[#a67c52]">{film.featuredRelease.eyebrow}</p><h3 className="mt-4 text-4xl font-black tracking-tight text-[#2b2119] md:text-6xl">{film.featuredRelease.title}</h3><p className="mt-5 max-w-2xl text-lg leading-8 text-[#76685d]">{film.featuredRelease.description}</p><span className="mt-7 inline-flex font-black text-[#6f4e37]">Watch the published short →</span></Link></section> : null}
