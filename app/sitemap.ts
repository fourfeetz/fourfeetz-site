...
const routes=["/en","/about","/characters",...characterDetails.map(c=>`/characters/${c.slug}`),"/contact","/videos","/insights","/insights/guides","/insights/news","/music","/privacy","/resources","/services","/terms","/tools",...resourceDetails.map(r=>`/resources/${r.slug}`),...practicalResources.map(r=>`/resources/${r.slug}`)];
